...
RUN git clone https://github.com/cubiq/ComfyUI_essentials /comfyui/custom_nodes/ComfyUI_essentials \
    ...
    && git checkout 9d9f4bedfc9f0321c19faf71855e228c93bd0dc9 \
    && pip install -r requirements.txt

RUN git clone https://github.com/PozzettiAndrea/ComfyUI-SAM3 /comfyui/custom_nodes/ComfyUI-SAM3 \
...
RUN git clone https://github.com/WASasquatch/was-node-suite-comfyui /comfyui/custom_nodes/was-node-suite-comfyui \
    ...
    && git checkout ea935d1044ae5a26efa54ebeb18fe9020af49a45 \
    && pip install -r requirements.txt

RUN git clone https://github.com/kijai/ComfyUI-KJNodes /comfyui/custom_nodes/ComfyUI-KJNodes \
    && cd /comfyui/custom_nodes/ComfyUI-KJNodes \
    && git checkout 1ee6e3abb5b27625fe3b0ebf58716e333416b960 \
    && pip install -r requirements.txt

# download FLUX.2 klein UNET
...
RUN for i in 1 2 3 4 5; do \
    HF_TOKEN=$HF_TOKEN comfy model download \
        ...
        --filename 'flux2-vae.safetensors' && break; \
    if [ $i -eq 5 ]; then echo "model-download failed after 5 attempts" >&2; exit 1; fi; \
    echo "attempt $i failed; retrying in $((i*10))s" >&2; sleep $((i*10)); \
    done

# user-provided input image
RUN wget --progress=dot:giga \
    -O '/comfyui/input/2N0A0836copy.webp' \
    "https://cool-anteater-319.convex.cloud/api/storage/7c1f1c9d-3db4-49cc-b63e-23af836072ba"
